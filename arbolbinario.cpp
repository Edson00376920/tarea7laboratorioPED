...
void BinarySearchTree::insertar(void)
{
    ...
    do{
        ...
        cout << "Desea digitar un dato?"<<endl;
        cin >> resp;
        if(resp == 'y'){
...
{
    borrarNodoRec(&pBinarySearchTree, dato);
}

void BinarySearchTree::borrarNodoRec(nodo **r, int dato)
{
    nodo *p;
    ...
    if(!(*r))
        ...
        if(dato < (*r)->dato)
            borrarNodoRec(&((*r)->subLeft), dato);
        else
            if(dato > (*r)->dato)
                borrarNodoRec(&((*r)->subRight), dato);
            else{ 
...
void BinarySearchTree::buscarNodo(int dato)
{
...
{
    if(!p)
        cout << dato << " " << "no se encuentra en el árbol" << endl << endl;
    else
        if(dato < p->dato)
            buscarN(p->subLeft, dato);
        else
            if(dato > p->dato)
                buscarN(p->subRight, dato);
            else
                cout << dato<< " " <<"se encuentra en el árbol" << endl << endl;
}
